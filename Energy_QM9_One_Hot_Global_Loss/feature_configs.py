...
# -------------------------
# Order in data_processing.py:
#   - one_hot_atom_type (ATOM_TYPE_DIM)
#   - one_hot_hybridization (HYB_DIM)
#   - is_aromatic (1)
#   - one_hot_total_h (TOTAL_H_DIM)
#   - one_hot_formal_charge (FORMAL_CHARGE_DIM)
#   - gasteiger_charge_scaled (1)  # in [-1, 1]
#   - one_hot_degree (DEGREE_DIM)
#   - is_in_ring (1)
#   - neighbor_counts_scaled (NUM_NEIGHBOR_FEATURES)  # in [0, 1]
#   - functional_group_multi_hot (NUM_FUNC_GROUPS)
...
# Directed bond node feature dim:
#   concat(source atom features, bond features)
LINE_NODE_FEATURE_DIM: int = TOTAL_FEATURE_DIMENSION + NUM_BOND_FEATURES

# Line-edge feature layout:
#   one_hot_hybridization(dst_atom) + bond_i(6) + bond_j(6)
...
# -------------------------
#
# Stored as `Data.g` with shape (1, GLOBAL_FEATURE_DIM) per molecule.
# Under PyG batching, `Batch.g` becomes (num_graphs, GLOBAL_FEATURE_DIM).
#
# We intentionally keep global features as continuous descriptor values.
# Normalization (StandardScaler) is applied in train.py by fitting on the TRAIN split only.

GLOBAL_DESC_NAMES: List[str] = [
    "MolWt",
    "MolLogP",
    "TPSA",
    "NumHDonors",
...
GLOBAL_DESC_DIM: int = len(GLOBAL_DESC_NAMES)
GLOBAL_FEATURE_DIM: int = GLOBAL_DESC_DIM
