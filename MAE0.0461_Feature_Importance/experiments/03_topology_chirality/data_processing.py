...
def _compute_topology_global_features(mol: Chem.Mol) -> np.ndarray:
    ring_info = mol.GetRingInfo()
    atom_rings = ring_info.AtomRings()
    ring_sizes = [len(ring) for ring in atom_rings]
    aromatic_ring_count = sum(
        1 for ring in atom_rings if all(mol.GetAtomWithIdx(atom_idx).GetIsAromatic() for atom_idx in ring)
    )
    ...
    topo = np.array(
        [
            ...
            float(sum(1 for size in ring_sizes if size == 3)),
            float(sum(1 for size in ring_sizes if size == 4)),
            float(sum(1 for size in ring_sizes if size == 5)),
            float(sum(1 for size in ring_sizes if size == 6)),
            float(sum(1 for size in ring_sizes if size >= 7)),
            float(aromatic_ring_count),
        ],
        dtype=np.float32,
    )
    return topo
...
def _compute_atom_ring_size_features(mol: Chem.Mol) -> List[List[float]]:
    num_atoms = mol.GetNumAtoms()
    ring_features = [[0.0] * NUM_RING_SIZE_FEATURES for _ in range(num_atoms)]
    for ring in mol.GetRingInfo().AtomRings():
        ...
        for atom_idx in ring:
            ring_features[atom_idx][bucket] = 1.0
    return ring_features


def _get_atom_chiral_features(atom: Chem.Atom) -> List[float]:
    cip_code = atom.GetProp("_CIPCode") if atom.HasProp("_CIPCode") else ""
    possible = float(atom.HasProp("_ChiralityPossible"))
    return [
        float(cip_code == "R"),
        float(cip_code == "S"),
        possible,
...
def compute_global_features(mol: Chem.Mol) -> torch.Tensor:
    ...
    topo = _compute_topology_global_features(mol)
    desc = np.concatenate([desc, topo], axis=0).astype(np.float32, copy=False)
...
def smiles_to_graph_data(smiles: str, labels: List[float], mol_id: Optional[int] = None) -> Optional[Tuple[Data, Data]]:
    ...
    ring_size_features = _compute_atom_ring_size_features(mol)
    ...
    for i in range(num_atoms):
        ...
        feat += _get_atom_chiral_features(atom)
        feat += ring_size_features[i]
