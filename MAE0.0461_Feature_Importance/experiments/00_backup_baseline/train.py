...
def select_device() -> torch.device:
    if torch.cuda.is_available():
        ...
        logging.info(
            "Execution device: GPU (cuda:%d, %s, compute %d.%d, %.1f GB VRAM)",
            device_index,
            props.name,
            int(props.major),
            int(props.minor),
        ...
        logging.info("CUDA is enabled. Dataset preprocessing before epoch 1 remains CPU-bound.")
        return torch.device(f"cuda:{device_index}")

    mps_backend = getattr(torch.backends, "mps", None)
    if mps_backend is not None and mps_backend.is_available():
        logging.info("Execution device: GPU (mps)")
        logging.info("MPS is enabled. Dataset preprocessing before epoch 1 remains CPU-bound.")
...
def train_one_run(
    ...
    train_loader, val_loader, test_loader = create_dataloaders(
        data_list, train_indices, val_indices, test_indices, hp["batch_size"], device=device
    )
    ...
    if bool(hp.get("enable_hard_mining", False)):
        ...
        if tr_pred_s.size and tr_true_s.size:
            ...
            if top_k > 0:
                ...
                stage2_train_loader = build_dataloader(
    ...
    if final_k > 0 and bool(hp.get("save_train_worstk_best_checkpoint", True)):
        train_eval_loader2 = build_dataloader(
            [data_list[i] for i in train_indices],
            batch_size=hp["batch_size"],
            shuffle=False,
            device=device,
...
def main() -> None:
    ...
    logging.info("Dataset ready in %.2fs.", time.perf_counter() - stage_started_at)
    if not data_list:
    ...
    logging.info("Data split ready in %.2fs.", time.perf_counter() - stage_started_at)
    ...
    logging.info("Label scaling ready in %.2fs.", time.perf_counter() - stage_started_at)
