...
def _get_morgan_generator(radius: int, fp_bits: int, use_chirality: bool):
    """Cache the Morgan generator to avoid re-allocations per molecule."""
    # rdFingerprintGenerator is optional (see import above)
...
def apply_global_scaling(
    ...
    for atom_g, line_g in data_list:
        if hasattr(atom_g, "g") and atom_g.g is not None:
            atom_g.g = (atom_g.g - mean_t) / std_t
        if hasattr(line_g, "g") and line_g.g is not None:
            line_g.g = (line_g.g - mean_t) / std_t
