...
def train_epoch(
    ...
    total_loss = 0.0
    total_graphs = 0
    use_non_blocking = getattr(device, "type", str(device)) == "cuda"
    ...
    for batch in tqdm(loader, desc="Training", leave=False):
        ...
        atom_batch = atom_batch.to(device, non_blocking=use_non_blocking)
        line_batch = line_batch.to(device, non_blocking=use_non_blocking)
        ...
        # Noam-style schedules update on every optimizer step.
        if scheduler is not None:
            scheduler.step()

        # Average by graph count so the last batch does not skew epoch loss.
...
def evaluate_metrics(
    ...
    property_names_used = property_names if len(property_names) == num_properties else [f"Property_{i}" for i in range(num_properties)]
...
def plot_results(
    ...
    if train_losses and val_losses:
        ...
        ax_loss.set_title("Training & Validation Loss Curve", fontsize=32)
    ...
    property_names_plot = property_names if len(property_names) == num_properties else [f"Target_{i+1}" for i in range(num_properties)]
    ...
    for i in range(num_properties):
        ...
        ax_scatter.set_ylabel("LOHCGNN Prediction", fontsize=36, fontweight="bold", labelpad=20)
        ...
        ax_scatter.text(
            ...
            bbox=dict(boxstyle="round,pad=0.5", fc="wheat", alpha=0.9, edgecolor="black", linewidth=1.5),
        )
