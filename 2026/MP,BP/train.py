...
from gnn_model import LOHCGNN, BaselineGCN, BaselineGIN
...
def build_model(num_node_features: int, hp: Dict[str, Any], device: torch.device) -> nn.Module:
    model_type = hp.get("model_type", "lohcgnn").lower()
    
    if model_type == "gcn":
        logging.info("Building Baseline GCN Model...")
        model = BaselineGCN(
        ...
        logging.info("Building Baseline GIN Model...")
        model = BaselineGIN(
            node_in_dim=num_node_features,
            hidden_dim=hp["hidden_dim"],
            num_layers=hp["num_layers"],
        ...
        logging.info("Building LOHCGNN Model...")
        
        use_le = hp.get("use_line_edge_features", True)
        actual_line_node_dim = TOTAL_FEATURE_DIMENSION + NUM_BOND_FEATURES
        actual_line_edge_dim = NUM_LINE_EDGE_FEATURES if use_le else 0

        model = LOHCGNN(
            node_in_dim=num_node_features,
            edge_in_dim=NUM_BOND_FEATURES,
            line_node_in_dim=actual_line_node_dim,
            line_edge_in_dim=actual_line_edge_dim,
            hidden_dim=hp["hidden_dim"],
            num_layers=hp["num_layers"],
            num_output_features=hp["num_output_features"],
            dropout_rate=hp["dropout_rate"],
            global_in_dim=GLOBAL_FEATURE_DIM,
            pooling_type="add", # 녹는점 예측을 위해 Mean Pooling 적용
            use_line_edge_features=use_le        
        ).to(device)
