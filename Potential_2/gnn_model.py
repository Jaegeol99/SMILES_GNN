...
class LOHCGNN(nn.Module):
    ...
    def forward(self, atom_data: Batch, line_data: Batch) -> torch.Tensor:
        ...
        att_logits_h = self.atom_att_pool.gate_nn(h_h)
        att_logits_d = self.atom_att_pool.gate_nn(h_d)

        h_h_alpha = softmax(att_logits_h, atom_data.batch)
        h_d_alpha = softmax(att_logits_d, atom_data.batch_de)

        # Attention Pooling
        h_h_att = self.atom_att_pool(h_h, atom_data.batch)
        h_d_att = self.atom_att_pool(h_d, atom_data.batch_de)
        l_h_att = self.line_att_pool(l_h, line_data.batch)
        l_d_att = self.line_att_pool(l_d, line_data.batch_de)

        atom_features = h_h_att - h_d_att
        line_features = l_h_att - l_d_att

        combined = torch.cat([atom_features, line_features], dim=-1)

        prediction = self.mlp(combined)

        return prediction, {
            'hydrogenated_atom_attention': h_h_alpha,
            'dehydrogenated_atom_attention': h_d_alpha
        }