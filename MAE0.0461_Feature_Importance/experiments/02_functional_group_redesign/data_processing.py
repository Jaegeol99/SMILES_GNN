...
from feature_configs import (
    ...
    FUNCTIONAL_GROUP_ROLE_FOCUS,
    FUNCTIONAL_GROUP_ROLE_NAMES,
    FUNCTIONAL_GROUP_ROLE_PATTERNS,
    GLOBAL_FEATURE_DIM,
    LAPLACIAN_PE_K,
...
def compute_global_features(mol: Chem.Mol) -> torch.Tensor:
    ...
    fg_counts = np.array(
        [float(len(mol.GetSubstructMatches(FUNCTIONAL_GROUP_PATTERNS[name]))) for name in FUNCTIONAL_GROUP_NAMES],
        dtype=np.float32,
    )
    denom = float(max(mol.GetNumAtoms(), 1))
    fg_densities = fg_counts / denom
    desc = np.concatenate([desc, fg_counts, fg_densities], axis=0).astype(np.float32, copy=False)
...
def smiles_to_graph_data(smiles: str, labels: List[float], mol_id: Optional[int] = None) -> Optional[Tuple[Data, Data]]:
    ...
    func_group_role_matches = {
        name: {
            match[FUNCTIONAL_GROUP_ROLE_FOCUS[name]]
            for match in mol.GetSubstructMatches(pat)
            if len(match) > FUNCTIONAL_GROUP_ROLE_FOCUS[name]
        }
        for name, pat in FUNCTIONAL_GROUP_ROLE_PATTERNS.items()
    }
    ...
    for i in range(num_atoms):
        ...
        feat += [1.0 if i in func_group_role_matches[name] else 0.0 for name in FUNCTIONAL_GROUP_ROLE_NAMES]
