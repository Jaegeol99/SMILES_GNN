...
def build_scheduler(optimizer: optim.Optimizer, hp: Dict[str, Any]):
    ...
    if sched == "plateau":
        return ReduceLROnPlateau(
            ...
            #[수정됨] 최신 PyTorch 버전 호환을 위해 verbose=False 제거
        )
    elif sched == "exponential":
        return ExponentialLR(optimizer, gamma=float(hp.get("scheduler_gamma", 0.95)))
    return None
                ...
                if stage2_epochs > 0:
                    ...
                    for e2 in range(stage2_epochs):
                        ...
                        if stage2_scheduler is not None:
                            if isinstance(stage2_scheduler, ReduceLROnPlateau):
